...
from feedback_bot.errors import ConfigError
...
class Config(object):
    def __init__(self, filepath):
        ...
        # Feedback_bot specific config
        self.management_room = self._get_cfg(["feedback_bot", "management_room"], required=True)
        self.management_room_id = self.management_room if self.management_room.startswith("!") else None
        self.anonymise_senders = self._get_cfg(["feedback_bot", "anonymise_senders"], required=False, default=False)
        self.welcome_message = self._get_cfg(["feedback_bot", "welcome_message"], required=False)
        self.mention_only_rooms = self._get_cfg(["feedback_bot", "mention_only_rooms"], required=False, default=[])
        self.mention_only_always_for_named = self._get_cfg(
            ["feedback_bot", "mention_only_always_for_named"], required=False, default=False,
        )
        self.confirm_reaction = self._get_cfg(["feedback_bot", "confirm_reaction", "enabled"], required=False, default=False)
        self.confirm_reaction_success = self._get_cfg(["feedback_bot", "confirm_reaction", "success"], required=False, default="✔️")
        self.confirm_reaction_fail = self._get_cfg(["feedback_bot", "confirm_reaction", "fail"], required=False, default="❗")
        self.relay_management_media = self._get_cfg(["feedback_bot", "relay_management_media"], required=False, default=False)
